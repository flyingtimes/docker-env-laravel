...
docker-compose run --rm composer sed -i "s/DB_DATABASE=/DB_DATABASE=homestead/g" /var/www/.env
docker-compose run --rm composer sed -i "s/DB_USERNAME=/DB_USERNAME=root/g" /var/www/.env
docker-compose run --rm composer sed -i "s/DB_PASSWORD=/DB_PASSWORD=secret/g" /var/www/.env
